...
class Database:
    ...
    def add_scheduled_reminder(self, student_id: int, schedule_id: int, reminder_time: datetime, lesson_time: datetime) -> bool:
        ...
        try:
            ...
            session.commit()
            print(f'[reminder] Добавлено напоминание в БД: student_id={student_id}, schedule_id={schedule_id}, reminder_time={reminder_time}')
            return True
        except Exception as e:
            session.rollback()
    ...
    def get_pending_reminders(self, current_time: datetime = None) -> list:
        """Получает все неотправленные напоминания, время которых уже наступило"""
    ...
    def mark_reminder_sent(self, reminder_id: int) -> bool:
        ...
        try:
            ...
            if reminder:
                ...
                session.commit()
                print(f'[reminder] Напоминание {reminder_id} отмечено как отправленное')
                return True
            return False
        except Exception as e:
            session.rollback()
    ...
    def clear_old_reminders(self, days: int = 7) -> int:
        ...
        try:
            ...
            session.commit()
            print(f'[reminder] Удалено {deleted_count} старых напоминаний')
    ...
    def delete_student_reminders(self, student_id: int) -> int:
        ...
        try:
            ...
            session.commit()
            print(f'[reminder] Удалено {deleted_count} напоминаний для student_id={student_id}')
            return deleted_count
        except Exception as e:
            session.rollback()
